...
Use Case [02]: dict object - remove additional sensitive keys with additional_key_patterns defined
...
- name: "Run test on dettonville.utils.remove_sensitive_keys"
  ignore_errors: yes
  ansible.builtin.set_fact:
    __test_filter_result: "{{ test_object | dettonville.utils.remove_sensitive_keys(key_patterns=[], additional_key_patterns=['(?i).*root.*']) }}"
