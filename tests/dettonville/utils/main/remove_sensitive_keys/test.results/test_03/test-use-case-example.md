...
Use Case [03]: dict object - remove sensitive keys with key_patterns explicitly defined
...
- name: "Run test on dettonville.utils.remove_sensitive_keys"
  ignore_errors: yes
  ansible.builtin.set_fact:
    __test_filter_result: "{{ test_object | dettonville.utils.remove_sensitive_keys(key_patterns=['(?i).*vault.*', '(?i).*token.*', '(?i).*password.*', '(?i).*key.*'], additional_key_patterns=[]) }}"
